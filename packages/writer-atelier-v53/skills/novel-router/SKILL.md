---
name: novel-router
description: Use when 소설·웹소설·팬픽·패러디·AU·크로스오버 작업을 시작하거나 이어가며 어떤 전문 소설 스킬을 불러야 할지 결정해야
  할 때.
metadata:
  version: 53.0.0
  display-name-ko: 소설 총괄 라우터
  role: router
---
# 소설 총괄 라우터 — 작가공방 V53
...
## V53 progressive disclosure
각 전문 스킬은 자기 폴더 안의 `references/`, `assets/`, `scripts/`만 필요할 때 읽는다. 라우터는 다른 스킬의 내부 파일을 직접 읽지 않고 **스킬 이름으로 책임을 인계**한다. 완성본 수정 단계의 횟수 통제에만 `references/REVISION_BUDGET_KO.md`와 `assets/revision-budget.schema.json`을 사용한다. 이 자원은 초고 Writer에게 상시 주입하지 않는다.
