...
def _tokens(text: str): return [m.group(0).lower() for m in WORD_RE.finditer(text)]
def _sentences(text: str): return [s.strip() for s in SENTENCE_RE.split(text.strip()) if s.strip()]
def _paragraphs(text: str): return [p.strip() for p in re.split(r"\n\s*\n", text.strip()) if p.strip()]
...
def phrase_echo_candidates(text: str, min_n: int = 2, max_n: int = 4):
    ...
    for n in range(min_n,max_n+1):
        for i in range(len(tokens)-n+1):
            ...
            if all(len(t)<=1 for t in gram): continue
            positions[(n,gram)].append(i)
    out=[]
    for (n,gram),pos in positions.items():
        if len(pos)<2: continue
        local=min((b-a for a,b in zip(pos,pos[1:])),default=9999)
        if local<=80: out.append({'phrase':' '.join(gram),'n':n,'count':len(pos),'min_token_gap':local,'token_positions':pos[:8]})
    out.sort(key=lambda x:(-x['count'],-x['n'],x['min_token_gap'],x['phrase']))
    return out[:40]

def sentence_start_candidates(text: str):
    starts=defaultdict(list); sents=_sentences(text)
    for i,s in enumerate(sents):
        toks=_tokens(s)
        if len(toks)>=2: starts[' '.join(toks[:2])].append(i)
        elif toks: starts[toks[0]].append(i)
    return sorted([{'start':k,'count':len(v),'sentence_indices':v[:10]} for k,v in starts.items() if len(v)>=2],key=lambda x:(-x['count'],x['start']))[:30]
...
def dialogue_rhythm_candidates(text: str):
    ...
    def flush():
        nonlocal run
        if len(run)>=4:
            lengths=[len(_tokens(paragraphs[i])) for i in run]; mean=sum(lengths)/len(lengths) if lengths else 0; stdev=statistics.pstdev(lengths) if len(lengths)>1 else 0
            if mean<=12 and stdev<=max(3,mean*0.45): flags.append({'paragraph_indices':run[:],'turn_count':len(run),'mean_words':round(mean,2),'word_length_stdev':round(stdev,2),'reason':'짧고 비슷한 길이의 대사 문단이 연속됨; 실제 상태 변화가 있는지 냉독 필요'})
        run=[]
...
def analyze_text(text: str):
    paragraphs=_paragraphs(text); sentences=_sentences(text); para_lengths=[len(_tokens(p)) for p in paragraphs]; sent_lengths=[len(_tokens(s)) for s in sentences]
...
def main():
    ap=argparse.ArgumentParser(description='소설 본문의 반복/리듬 검토 후보를 JSON으로 출력합니다.'); ap.add_argument('file',nargs='?',help='UTF-8 본문 파일. 생략하면 stdin 사용'); ap.add_argument('--output',help='JSON 저장 경로'); args=ap.parse_args()
